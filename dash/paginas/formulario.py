...
caminho_modelo = os.path.join(os.path.dirname(__file__), '..', 'modelo_xgboost.pkl')
caminho_mediana = os.path.join(os.path.dirname(__file__), '..' ,'mediana.pkl')

# Carregar modelo e medianas uma única vez (evita recarregamento desnecessário)
modelo = joblib.load(caminho_modelo)
medianas = joblib.load(caminho_mediana)
...
def prever_doenca(n_clicks, *args):
    ...
    # Criando DataFrame com os valores do formulário
    colunas = ['age', 'sex', 'cp', 'trestbps', 'chol', 'fbs', 'restecg', 'thalach', 'exang', 'oldpeak', 'slope', 'ca', 'thal']
    entradas_usuario = pd.DataFrame([args], columns=colunas)

    # Substituir valores `None` pelas medianas corretamente
    for coluna in colunas:
        if entradas_usuario[coluna].isna().any():  # Se houver `None`, substituir pela mediana
            entradas_usuario[coluna] = entradas_usuario[coluna].fillna(medianas[coluna]).infer_objects(copy=False)
